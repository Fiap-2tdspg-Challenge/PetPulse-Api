...
public class ValidationHandler {
    ...
    public record ConflictErrorResponse(String message) {
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    @ResponseStatus(HttpStatus.CONFLICT)
    public ConflictErrorResponse handleDataIntegrityViolation(
            DataIntegrityViolationException exception
    ) {

        log.warn("Erro de integridade de dados: {}", exception.getMessage());

        return new ConflictErrorResponse(
                "Não é possível excluir este registro porque ele possui dados relacionados."
        );
    }
}